 ...
 *    pos / 4 的整数就是最大单位
 */
export const getAmountChinese = (val: string) => {
  const amount = +val;
 ...
 * @param end 结束时间
 */
export const timeCalculate = (start: number, end: number) => {
 ...
 * @param gap
 */
export const debounce = (func: Function, gap: number) => {
  let timer;
 ...
 * @returns {Array} 新的数组
 */
export const mergeNumberOfExps = (expressions: Array<any>): Array<any> => {
  const res = [];
  const isNumChar = (n: string) => /^[\d|.]$/.test(n);
 ...
 * @returns {Boolean}
 */
export const validExp = (expressions: Array<any>, mergeNum = true): boolean => {
 ...
 */
export const toRPN = (exps: Array<any>) => {
  const s1 = []; // 符号栈
  const s2 = []; // 输出栈
 ...
 */
export const calcRPN = (rpnExps: Array<any>) => {
