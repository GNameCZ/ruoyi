<template>
  ...
  <el-form-item v-if="props.data.layout !== 'rowFormItem' && props.data.layout !== undefined && !props.data.customDefaultValue" label="默认值">
    <el-input v-if="props.data.tag === 'my-text'" v-model="props.data.defaultValue" placeholder="请输入文本内容" />

    <component :is="props.data.tag" v-else v-model="props.data.defaultValue" style="width: 100%" v-bind="props.data"> </component>
  </el-form-item>
  ...
  <el-form-item v-if="props.data.asSummary !== undefined" label="作为摘要">
    <el-switch v-model="props.data.asSummary" />
  </el-form-item>

  <el-form-item v-if="props.data.cmpType === 'custom'" label="组件说明">
    <el-input v-model="props.data.explain" type="textarea" />
  </el-form-item>

  <el-form-item v-if="props.data.textAlign !== undefined" label="对齐方式">
  ...
  <el-form-item v-if="props.data.required !== undefined" label="是否必填">
    <el-switch v-model="props.data.required" :disabled="!props.couldChangeRequire" @change="requireChange" />
