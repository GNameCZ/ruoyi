/** *Copyright: Copyright (c) 2023 *Author:Leo *Title: 表单设计器 *GitHub: https://github.com/leooneone/ai-form */

<template>
  <div style="height: 100%; background-color: white">
    <el-container style="height: 100%">
      <el-header v-if="props.showHeader" style="height: 50px; display: flex; -webkit-box-shadow: 0 2px 8px #f0f1f2; box-shadow: 0 2px 8px #f0f1f2">
        <span style="align-self: center; font-size: 20px; color: rgba(0, 0, 0, 0.7)">
      ...
      <el-main style="padding: 0">
        <el-container style="height: 100%">
          <el-aside width="300px" class="left-panel">
            <el-tabs v-model="state.activeTabName" :stretch="true" style="height: 100%">
              <el-tab-pane v-for="(item, index) in state.comTabs" :key="index" :label="item.label" :name="item.name">
                <div class="components-list">
                  <template v-for="com in item.coms">
                      ...
                      <template #item="{ element, index }">
                        <div :key="index" class="components-item" @click="addComponent(element)">
                          <div class="components-body">
                            <ai-svg-icon :icon-class="element.tagIcon" class="svg-icon" size="18" :color="com.color" />
                            {{ element.label }}
                          </div>
                        </div></template
                      >
          ...
          <el-main>
            <el-container class="center-panel" style="height: 100%">
              <el-header class="btn-bar" style="height: 45px">
                <el-button-group>
                  <el-button icon="Cellphone" title="手机" @click="setMode('h5')">手机 </el-button>

                  <el-button icon="Monitor" title="电脑" @click="setMode('pc')">电脑</el-button>
                  <el-button icon="View" plain title="预览" @click="preview">预览</el-button>
                  <el-button icon="Delete" plain title="清空" @click="empty">清空</el-button>
                </el-button-group>
              </el-header>
          ...
          <el-aside class="right-panel"
            ><el-scrollbar>
              <right-panel
                ...
                ><slot name="right" :active-data="state.activeData"></slot></right-panel
            ></el-scrollbar>
          </el-aside>
        </el-container>
      </el-main>
    ...
    <el-dialog v-model="state.isErrorShow" title="验证提示">
      <el-alert v-for="(item, index) in state.validResults" :key="index" type="error" :title="item" style="margin-bottom: 10px"></el-alert>
...
//当list改变时
const handlerListChange: Function = (val) => {
  clearPCondition();
  //是否可以用作流程条件
  const canUsedAsPCon = (conf, parent) => {
    const isRangeCmp = ['ai-date-duration', 'ai-time-duration'].includes(conf.tag);
