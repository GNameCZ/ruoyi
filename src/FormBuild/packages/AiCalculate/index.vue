  ...
  try {
    if (vModel.indexOf('.') > -1) {
      ...
      if (typeof props.rowIndex === 'number') {
        ...
        return rootFormData.value[tabelVModel].reduce((sum, c) => (c[cmpVModel] || 0) + sum, 0);
      }
    }
 ...
 * 计算表达式
 */
const execRPN: Function = () => {
  const temp = state.RPN_EXP.map((t) => (typeof t === 'object' ? getFormVal(t.vModel) : t));
